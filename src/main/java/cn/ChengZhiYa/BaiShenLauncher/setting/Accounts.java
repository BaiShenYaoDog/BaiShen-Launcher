...
import cn.ChengZhiYa.BaiShenLauncher.auth.authlibinjector.*;
...
import cn.ChengZhiYa.BaiShenLauncher.auth.microsoft.MicrosoftService;
...
import cn.ChengZhiYa.BaiShenLauncher.auth.yggdrasil.RemoteAuthenticationException;
...
public final class Accounts {
    ...
    public static final List<AccountFactory<?>> FACTORIES = Lang.immutableListOf(FACTORY_OFFLINE, FACTORY_MOJANG, FACTORY_MICROSOFT, FACTORY_AUTHLIB_INJECTOR);
    // ==== Login type name i18n ===
    private static final Map<AccountFactory<?>, String> unlocalizedLoginTypeNames = Lang.mapOf(
            Pair.pair(Accounts.FACTORY_OFFLINE, "离线账户"),
            ...
            Pair.pair(Accounts.FACTORY_MICROSOFT, "account.methods.microsoft"));
    ...
    private static void loadGlobalAccountStorages() {
        ...
        if (Files.exists(globalAccountsFile)) {
            try (Reader reader = Files.newBufferedReader(globalAccountsFile)) {
                ...
                Logging.LOG.log(Level.WARNING, "Failed to load global accounts", e);
            }
        }

        InvocationDispatcher<String> dispatcher = InvocationDispatcher.runOn(Lang::thread, json -> {
            Logging.LOG.info("Saving global accounts");
            synchronized (globalAccountsFile) {
                try {
                    ...
                    Logging.LOG.log(Level.SEVERE, "Failed to save global accounts", e);
                }
            }
        });
        ...
        if (selected != null) {
            ...
            Schedulers.io().execute(() -> {
                try {
                    ...
                    Logging.LOG.log(Level.WARNING, "Failed to log " + finalSelected + " in", e);
                }
            });
        }
