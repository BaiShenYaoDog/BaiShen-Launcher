...
public class GameCrashWindow extends Stage {
    ...
    private final TextFlow reasonTextFlow = new TextFlow(new Text(I18n.i18n("game.crash.reason.unknown")));
    ...
    public GameCrashWindow(ManagedProcess managedProcess, ProcessListener.ExitType exitType, DefaultGameRepository repository, Version version, LaunchOptions launchOptions, Collection<Pair<String, Log4jLevel>> logs) {
        ...
        this.feedbackTextFlow.getChildren().addAll(FXUtils.parseSegment("未知的崩溃原因!", Controllers::onHyperlinkAction));
    ...
    private void analyzeCrashReport() {
        ...
        CompletableFuture.supplyAsync(() -> {
            ...
            if (exception != null) {
                Logging.LOG.log(Level.WARNING, "Failed to analyze crash report", exception);
                reasonTextFlow.getChildren().setAll(FXUtils.parseSegment(I18n.i18n("game.crash.reason.unknown"), Controllers::onHyperlinkAction));
            } else {
                ...
                for (CrashReportAnalyzer.Result result : results) {
                    switch (result.getRule()) {
                        case TOO_OLD_JAVA:
                            segments.addAll(FXUtils.parseSegment(I18n.i18n("game.crash.reason.too_old_java",
                        ...
                        case MOD_RESOLUTION_COLLECTION:
                            segments.addAll(FXUtils.parseSegment(I18n.i18n("game.crash.reason." + result.getRule().name().toLowerCase(Locale.ROOT),
                ...
                if (results.isEmpty()) {
                    if (!keywords.isEmpty()) {
                        reasonTextFlow.getChildren().setAll(new Text(I18n.i18n("game.crash.reason.stacktrace", String.join(", ", keywords))));
                    } else {
                        reasonTextFlow.getChildren().setAll(FXUtils.parseSegment(I18n.i18n("game.crash.reason.unknown"), Controllers::onHyperlinkAction));
                    }
    ...
    private String parseFabricModId(String modName) {
        ...
        if (matcher.find()) {
            ...
            if ("[*]".equals(version)) {
                return I18n.i18n("game.crash.reason.mod_resolution_mod_version.any", translateFabricModId(modid));
            } else {
                return I18n.i18n("game.crash.reason.mod_resolution_mod_version", translateFabricModId(modid), version);
            }
        }
    ...
    private void exportGameCrashInfo() {
        ...
        CompletableFuture.supplyAsync(() ->
                ...
                .thenRunAsync(() -> {
                    ...
                    Alert alert = new Alert(Alert.AlertType.INFORMATION, I18n.i18n("settings.launcher.launcher_log.export.success", logFile));
                    alert.setTitle(I18n.i18n("settings.launcher.launcher_log.export"));
                    alert.showAndWait();
                }, Schedulers.javafx())
                .exceptionally(e -> {
                    Logging.LOG.log(Level.WARNING, "Failed to export game crash info", e);
                    return null;
                });
    }
