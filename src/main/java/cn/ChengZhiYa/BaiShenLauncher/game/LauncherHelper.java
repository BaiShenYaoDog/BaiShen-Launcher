...
import cn.ChengZhiYa.BaiShenLauncher.auth.authlibinjector.AuthlibInjectorDownloadException;
...
import cn.ChengZhiYa.BaiShenLauncher.download.MaintainTask;
import cn.ChengZhiYa.BaiShenLauncher.download.game.*;
import cn.ChengZhiYa.BaiShenLauncher.download.java.JavaRepository;
import cn.ChengZhiYa.BaiShenLauncher.launch.*;
...
import cn.ChengZhiYa.BaiShenLauncher.task.*;
...
import cn.ChengZhiYa.BaiShenLauncher.util.platform.*;
...
public final class LauncherHelper {
        ...
        return Task.composeAsync(() -> {
            ...
            if (javaVersion == null) {
                ...
                if (setting.isJavaAutoSelected()) {
                    ...
                    if (cn.ChengZhiYa.BaiShenLauncher.util.platform.Platform.isCompatibleWithX86Java()) {
                    ...
                    if (targetJavaVersion == null) {
                        Controllers.confirm(I18n.i18n("launch.failed.no_accepted_java"), I18n.i18n("message.warning"), MessageType.WARNING, continueAction, () -> {
                        ...
                        downloadJava(gameVersion.toString(), targetJavaVersion, profile)
                                ...
                                .exceptionally(throwable -> {
                                    Logging.LOG.log(Level.WARNING, "Failed to download java", throwable);
                                    Controllers.confirm(I18n.i18n("launch.failed.no_accepted_java"), I18n.i18n("message.warning"), MessageType.WARNING, continueAction, () -> {
                                        future.completeExceptionally(new CancellationException("No accepted java"));
                                    });
                                    return null;
                    ...
                    Controllers.dialog(I18n.i18n("launch.wrong_javadir"), I18n.i18n("message.warning"), MessageType.WARNING, continueAction);
                ...
                if (suggestedJavaVersion != null) {
                    ...
                    switch (violatedMandatoryConstraint) {
                        ...
                        case VANILLA_JAVA_16:
                            Controllers.confirm("Minecraft " + gameVersion + "至少需要Java " + 16 + "才能启动\n但是豹豹没有找到这个版本的Java\n点击确定豹豹将会自动下载Java", I18n.i18n("message.warning"),
                                    () -> FXUtils.openLink(OPENJDK_DOWNLOAD_LINK), null);
                            breakAction.run();
                            return Task.fromCompletableFuture(future);
                        case VANILLA_JAVA_17:
                            Controllers.confirm("Minecraft " + gameVersion + "至少需要Java " + 17 + "才能启动\n但是豹豹没有找到这个版本的Java\n点击确定豹豹将会自动下载Java", I18n.i18n("message.warning"),
                                    () -> FXUtils.openLink(OPENJDK_DOWNLOAD_LINK), null);
                            breakAction.run();
                            return Task.fromCompletableFuture(future);
                        case VANILLA_JAVA_8:
                            Controllers.dialog("Minecraft 1.13到1.16都必须使用Java 8或更高版本的Java才能运行!", I18n.i18n("message.error"), MessageType.ERROR, breakAction);
                            return Task.fromCompletableFuture(future);
                        case VANILLA_LINUX_JAVA_8:
                            if (setting.getNativesDirType() == NativesDirectoryType.VERSION_FOLDER) {
                                Controllers.dialog(I18n.i18n("launch.advice.vanilla_linux_java_8"), I18n.i18n("message.error"), MessageType.ERROR, breakAction);
                                return Task.fromCompletableFuture(future);
                            } else {
                                break;
                            }
                        case LAUNCH_WRAPPER:
                            Controllers.dialog(I18n.i18n("请使用Java 8而不是Java 9或以上") + "\n" + I18n.i18n("launch.advice.uncorrected"), I18n.i18n("message.error"), MessageType.ERROR, breakAction);
                            return Task.fromCompletableFuture(future);
                    }
                }
            }
            ...
            if (violatedSuggestedConstraints != null) {
                for (JavaVersionConstraint violatedSuggestedConstraint : violatedSuggestedConstraints) {
                    switch (violatedSuggestedConstraint) {
                        ...
                        case VANILLA_X86:
                            if (setting.getNativesDirType() == NativesDirectoryType.VERSION_FOLDER
                                    && cn.ChengZhiYa.BaiShenLauncher.util.platform.Platform.isCompatibleWithX86Java()) {
            ...
            if (suggestions.isEmpty()) {
                ...
                if (suggestions.size() == 1) {
                    message = I18n.i18n("launch.advice", suggestions.get(0));
                } else {
                    message = I18n.i18n("launch.advice.multi", suggestions.stream().map(it -> "→ " + it).collect(Collectors.joining("\n")));
                }

                Controllers.confirm(message, I18n.i18n("message.warning"), MessageType.WARNING, () -> future.complete(javaVersion), breakAction);
            }
    ...
    private static CompletableFuture<JavaVersion> downloadJava(String gameVersion, GameJavaVersion javaVersion, Profile profile) {
        ...
        Controllers.dialog(new MessageDialogPane.Builder(
                "Minecraft " + gameVersion + "至少需要Java " + javaVersion.getMajorVersion() + "才能启动\n但是豹豹没有找到这个版本的Java\n点击确定豹豹将会自动下载Java",
                I18n.i18n("message.warning"),
        ...
        return Task.composeAsync(() -> {
            try {
                ...
                runInFX(() -> {
                    ...
                    Controllers.dialog(new MessageDialogPane.Builder(I18n.i18n("account.failed.server_disconnected"), I18n.i18n("account.failed"), MessageType.ERROR)
                        ...
                        if (!success) {
                            ...
                            if (!(ex instanceof CancellationException)) {
                                ...
                                if (ex instanceof ModpackCompletionException) {
                                    ...
                                    Controllers.prompt(new PromptDialogPane.Builder(I18n.i18n("launch.failed.execution_policy"),
                                    ...
                                    message = I18n.i18n("exception.access_denied", ((AccessDeniedException) ex).getFile());
                                } else {
    ...
    private final class HMCLProcessListener implements ProcessListener {
        ...
        public HMCLProcessListener(HMCLGameRepository repository, Version version, AuthInfo authInfo, LaunchOptions launchOptions, CountDownLatch launchingLatch, boolean detectWindow) {
            ...
            final int numLogs = ConfigHolder.config().getLogLines() + 1;
        ...
        public void onExit(int exitCode, ExitType exitType) {
            ...
            if (exitType != ExitType.NORMAL) {
                ...
                Lang.forEachZipped(logs, levels,
                        (log, l) -> pairs.add(Pair.pair(log, l instanceof Log4jLevel ? ((Log4jLevel) l) : Optional.ofNullable(Log4jLevel.guessLevel(log)).orElse(Log4jLevel.INFO))));
