...
if [[ "$_HMCL_OS" == "unknown" || "$_HMCL_ARCH" == "unknown" ]]; then
  if [ "$_HMCL_USE_CHINESE" == true ]; then
    echo "运行 HMCL 需要 BaiShen-Launcher 运行时环境，请安装 Java 并设置环境变量后重试。" 1>&2
  else
    echo "运行 HMCL 需要 BaiShen-Launcher 运行时环境，请安装 Java 并设置环境变量后重试。" 1>&2
  fi
  exit 1
...
if [[ "$_HMCL_ARCH" == "loongarch64" ]]; then
  if [ "$_HMCL_USE_CHINESE" == true ]; then
    echo "运行 BaiShen-Launcher 需要 Java 运行时环境，请安装龙芯 JDK8 (https://docs.hmcl.net/downloads/loongnix.html) 并设置环境变量后重试。" 1>&2
  else
    echo "运行 BaiShen-Launcher 需要 Java 运行时环境，请安装龙芯 JDK8 (https://docs.hmcl.net/downloads/loongnix.html) 并设置环境变量后重试。" 1>&2
  fi
  exit 1
